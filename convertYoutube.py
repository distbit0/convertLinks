...
def convertYoutube(video_url, forceRefresh):
    ...
    for entry in transcript:
        ...
        if group_word_count >= 80:
            text = ' '.join(group)
            markdown_transcript += f"[{group_start_time}]({video_url}&t={group_start_time}): {text}\n\n"
            group = []
            group_word_count = 0
            group_start_time = None
    if group:
        text = ' '.join(group)
        markdown_transcript += f"[{group_start_time}]({video_url}&t={group_start_time}): {text}\n\n"
    title = transcript_list._manually_created_transcripts[0].title if transcript_list._manually_created_transcripts else inputSource
...
if __name__ == "__main__":
    print(convertYoutube("https://www.youtube.com/watch?v=eye33kD4uSI###convert", True))
